    ...
    ui(new Ui::pinewoodPC)
{
    ...
    connect(ui->pushButton_6,SIGNAL(clicked()),this,SLOT(reRace()));
}
...
bool pinewoodPC::isPriorityRacer()
{
    ...
    for (int i=0;i<ctrl->getCounter();i++)
        if (ctrl->getRacerByIndex(i).getPriority()==true && ctrl->getRacerByIndex(i).getT1()==0)
            j++;
    if (j==0)
        exists=false;
    else
...
QString pinewoodPC::getNextRacer(int Time, bool Priority)
{
    ...
    for(int i=0; i<ctrl->getCounter();i++){
        switch (Time){
        case 1:
            if (ctrl->getRacerByIndex(i).getName()!=prev1 && ctrl->getRacerByIndex(i).getName()!=prev2 && ctrl->getRacerByIndex(i).getName()!=prev3 && ctrl->getRacerByIndex(i).getName()!=prev4 && ctrl->getRacerByIndex(i).getT1()==0 && ctrl->getRacerByIndex(i).getPriority()==Priority)
                return ctrl->getRacerByIndex(i).getName();
            break;
        case 2:
            if (ctrl->getRacerByIndex(i).getName()!=prev1 && ctrl->getRacerByIndex(i).getName()!=prev2 && ctrl->getRacerByIndex(i).getName()!=prev3 && ctrl->getRacerByIndex(i).getName()!=prev4 && ctrl->getRacerByIndex(i).getT2()==0 && ctrl->getRacerByIndex(i).getPriority()==Priority)
                return ctrl->getRacerByIndex(i).getName();
            break;
        case 3:
            if (ctrl->getRacerByIndex(i).getName()!=prev1 && ctrl->getRacerByIndex(i).getName()!=prev2 && ctrl->getRacerByIndex(i).getName()!=prev3 && ctrl->getRacerByIndex(i).getName()!=prev4 && ctrl->getRacerByIndex(i).getT3()==0 && ctrl->getRacerByIndex(i).getPriority()==Priority)
                return ctrl->getRacerByIndex(i).getName();
            break;
        case 4:
            if (ctrl->getRacerByIndex(i).getName()!=prev1 && ctrl->getRacerByIndex(i).getName()!=prev2 && ctrl->getRacerByIndex(i).getName()!=prev3 && ctrl->getRacerByIndex(i).getName()!=prev4 && ctrl->getRacerByIndex(i).getT4()==0 && ctrl->getRacerByIndex(i).getPriority()==Priority)
                return ctrl->getRacerByIndex(i).getName();
            break;
        default:
            break;
...
void pinewoodPC::nextrace()
{
    if (ui->actionChampionship->isEnabled()){
        ...
        ui->pushButton->setEnabled(true);}
                ...
                ui->label->setText("__________");
                ui->label_2->setText("__________");
                ui->label_3->setText("__________");
                ui->label_4->setText("__________");
        int j=0;
        int aaa=currentT;
        //        for(int i=0;i<ctrl->getCounter();i++)
        switch(aaa){
        case 1:
            //            j=0;
            for(int k=0;k<ctrl->getCounter();k++)
                if(ctrl->getRacerByIndex(k).getT1()==0)
                    j++;
            if(j==0){
                currentT=2;}
            break;
        case 2:
            //            j=0;
            for(int k=0;k<ctrl->getCounter();k++)
                if(ctrl->getRacerByIndex(k).getT2()==0)
                    j++;
            if(j==0)
                currentT=3;
            break;
        case 3:
            //            j=0;
            for(int k=0;k<ctrl->getCounter();k++)
                if(ctrl->getRacerByIndex(k).getT3()==0)
                    j++;
            if(j==0)
                currentT=4;
            break;
        case 4:
            //            j=0;
            for(int k=0;k<ctrl->getCounter();k++)
                if(ctrl->getRacerByIndex(k).getT4()==0)
                    j++;
            if(j==0){
                if(languageCode==0)
                    QMessageBox::information(this,"Done","Championship finished!",QMessageBox::Ok);
                else
                    QMessageBox::information(this,"Kész","A bajnokságnak vége!",QMessageBox::Ok);
            }
            break;
        }
        ...
        if (ui->listWidget->count()!=0){
            for (int i=0;i<ctrl->getCounter();i++){
                if (ctrl->getRacerByIndex(i).getName()==ui->listWidget->item(0)->data(0))
                    switch (currentT){
                    case 1:
                        ctrl->getRacerByIndex_Pointer(i)->setT1(a1);
                        break;
                    case 2:
                        ctrl->getRacerByIndex_Pointer(i)->setT2(a1);
                        break;
                    case 3:
                        ctrl->getRacerByIndex_Pointer(i)->setT3(a1);
                        break;
                    case 4:
                        ctrl->getRacerByIndex_Pointer(i)->setT4(a1);
                        break;
                    }
            }

            for (int i=0;i<ctrl->getCounter();i++){
                if (ctrl->getRacerByIndex(i).getName()==ui->listWidget->item(1)->data(0))
                    switch (currentT){
                    case 1:
                        ctrl->getRacerByIndex_Pointer(i)->setT1(b1);
                        break;
                    case 2:
                        ctrl->getRacerByIndex_Pointer(i)->setT2(b1);
                        break;
                    case 3:
                        ctrl->getRacerByIndex_Pointer(i)->setT3(b1);
                        break;
                    case 4:
                        ctrl->getRacerByIndex_Pointer(i)->setT4(b1);
                        break;
                    }
            }

            for (int i=0;i<ctrl->getCounter();i++){
                if (ctrl->getRacerByIndex(i).getName()==ui->listWidget->item(2)->data(0))
                    switch (currentT){
                    case 1:
                        ctrl->getRacerByIndex_Pointer(i)->setT1(c1);
                        break;
                    case 2:
                        ctrl->getRacerByIndex_Pointer(i)->setT2(c1);
                        break;
                    case 3:
                        ctrl->getRacerByIndex_Pointer(i)->setT3(c1);
                        break;
                    case 4:
                        ctrl->getRacerByIndex_Pointer(i)->setT4(c1);
                        break;
                    }
            }

            for (int i=0;i<ctrl->getCounter();i++){
                if (ctrl->getRacerByIndex(i).getName()==ui->listWidget->item(3)->data(0))
                    switch (currentT){
                    case 1:
                        ctrl->getRacerByIndex_Pointer(i)->setT1(d1);
                        break;
                    case 2:
                        ctrl->getRacerByIndex_Pointer(i)->setT2(d1);
                        break;
                    case 3:
                        ctrl->getRacerByIndex_Pointer(i)->setT3(d1);
                        break;
                    case 4:
                        ctrl->getRacerByIndex_Pointer(i)->setT4(d1);
                        break;
                    }
            }
        }
...
void pinewoodPC::championship()
{
    ...
    ui->label->show();
    ui->listWidget->show();
    ui->pushButton_2->hide();//itt megcsereltem
    ui->pushButton_3->hide();//itt megcsereltem
...
void pinewoodPC::timeverify()
{
    ...
    switch(currentT){
    case 1:
        ui->label->setText(ui->listWidget->item(ui->lineEdit->text().toInt()-1)->data(0).toString());
        ui->label_2->setText(ui->listWidget->item(ui->lineEdit_2->text().toInt()-1)->data(0).toString());
        ui->label_3->setText(ui->listWidget->item(ui->lineEdit_3->text().toInt()-1)->data(0).toString());
        ui->label_4->setText(ui->listWidget->item(ui->lineEdit_4->text().toInt()-1)->data(0).toString());
        break;
    case 2:
        ui->label_4->setText(ui->listWidget->item(ui->lineEdit->text().toInt()-1)->data(0).toString());
        ui->label->setText(ui->listWidget->item(ui->lineEdit_2->text().toInt()-1)->data(0).toString());
        ui->label_2->setText(ui->listWidget->item(ui->lineEdit_3->text().toInt()-1)->data(0).toString());
        ui->label_3->setText(ui->listWidget->item(ui->lineEdit_4->text().toInt()-1)->data(0).toString());
        break;
    case 3:
        ui->label->setText(ui->listWidget->item(ui->lineEdit->text().toInt()-1)->data(0).toString());
        ui->label_2->setText(ui->listWidget->item(ui->lineEdit_2->text().toInt()-1)->data(0).toString());
